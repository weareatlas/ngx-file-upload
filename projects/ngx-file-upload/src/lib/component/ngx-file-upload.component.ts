...
export class NgxFileUploadComponent implements AfterViewInit, OnDestroy, OnChanges {
    ...
    click(fileUpload: IFileUpload, event: any) {
        event.stopPropagation();
        this.clicked.emit(fileUpload);
    }

    delete(fileUpload: IFileUpload, event?: any) {
        if (event) {
            event.stopPropagation();
        }
